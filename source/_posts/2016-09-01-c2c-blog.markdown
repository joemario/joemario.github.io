...
If you do, it's helpful to bump the kernel's perf sample rate up with the following two echo commands.  (see dmesg for "perf interrupt took too long ..." sample lowering entries).  
   ...
   so many samples that the perf tool can consume lots of cpu time and the perf.data file grows significantly.  
   Some tips to help that include:  
    - Bump the ldlat from the default of 30 to 50.  This free's perf to skip faster non-interesting loads.   
...
Although Don Zickus, Dick Fowles and I worked together to get this implemented,
